...
impl SiteResource {
    ...
    fn write_blog(&self, build_dir: &Path) -> OResult<()> {
        ...
        let blog_file = build_dir.join(BLOG_DIR)
            .join(&self.name)
            .with_extension(HTML);
    ...
    fn write_photo(&self, build_dir: &Path) -> OResult<()> {
        ...
        let thumbnail_path = build_dir.join(THUMBNAIL_DIR)
            .join(&self.name)
            .with_extension(JPG);
...
impl SiteResources {
    ...
    pub fn build_all(&self, build_dir: &Path, ignore_changed: bool) -> OResult<()> {
        info!("Creating build, build/{}, build/{}, and build/{}",
              BLOG_DIR, IMAGE_DIR, THUMBNAIL_DIR);
        fs::create_dir_all(build_dir)?;
        fs::create_dir_all(build_dir.join(BLOG_DIR))?;
